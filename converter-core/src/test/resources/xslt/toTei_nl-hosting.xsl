...
<xsl:output method="xml" indent="yes"/>
   ...
   <xsl:template match="/">
      ...
      <xsl:template match="ocr:page">
           <xsl:apply-templates />
        <pb/>
        <milestone n="{@physicalNumber}" style="height:{@height}px; width:{@width}px" type="page"/>
      </xsl:template>
      	...
      	<xsl:if test="$contextNode/@left">
      		<xsl:attribute name="style">
                <xsl:text>left:</xsl:text>
      			<xsl:value-of select="$contextNode/@left"/><xsl:text>px; </xsl:text>
                <xsl:text>top:</xsl:text>
      			<xsl:value-of select="$contextNode/@top"/><xsl:text>px; </xsl:text>
                <xsl:text>right:</xsl:text>
      			<xsl:value-of select="$contextNode/@right"/><xsl:text>px; </xsl:text>
                <xsl:text>bottom:</xsl:text>
      			<xsl:value-of select="$contextNode/@bottom"/><xsl:text>px</xsl:text>
      		</xsl:attribute>
      	</xsl:if>
      </xsl:template>
